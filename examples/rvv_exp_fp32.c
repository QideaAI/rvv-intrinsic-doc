...
//golden exp function
float exp_golden(float *x, float *y, float x_max, int N) {
    int i;
    float esum = 0.0f;

    for(i = 0; i < N; i ++) {
        x[i] -= x_max;
        y[i] = quick_dirty_expf(x[i]);
        esum += y[i];
        x[i] = exp(x[i]);
    }
...
//vector exp function
float exp_vec(float *x, float *y, float x_max, int N) {
    // values determined using (python)sollya
    const float ln2 = 0x1.62e43p-1;    
    const float iln2 = 0x1.715476p0f;
...
int main() {
    ...
    float x_max = 0.0f;
    float esum_golden = exp_golden(X, Y_golden, x_max, N);
    int pass = 1;
    for (int i = 0; i < N; i++) {
        if (!fp_eq(X[i], Y_golden[i], 1e-7)) {
    ...
    if (!pass) {
        ...
        float l2_dist = l2_distance(X, Y, N);
        float cos_sim = cos_similarity(X, Y, N);
        printf("pass!\n");
