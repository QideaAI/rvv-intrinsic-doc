...
void mul_golden(_Float16 *a, _Float16 *b, _Float16 *c, int N) {
    int i;
    for(i = 0; i < N; i ++) {
        c[i] = a[i] * b[i];
    }
...
//vector elementwise mul function
void mul_vec(_Float16 *a, _Float16 *b, _Float16 *c, int N) {
    for (size_t vl; N > 0; N -= vl, a += vl, b += vl, c += vl) {
        ...
        vfloat16m1_t vec_c = __riscv_vfmul_vv_f16m1(vec_a, vec_b, vl);
...
int main() {
    ...
    int pass = 1;
    for (int i = 0; i < N; i++) {
        if (!fp16_eq(C_golden[i], C[i], 2e-4)) {
